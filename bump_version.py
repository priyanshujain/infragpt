...
def get_current_version():
    init_file = Path("infragpt/__init__.py")
    ...
    if not match:
        raise ValueError("Could not find version string in infragpt/__init__.py")
...
def commit_and_tag(version):
    # Commit changes
    subprocess.run(["git", "add", "infragpt/__init__.py", "setup.py"], check=True)
